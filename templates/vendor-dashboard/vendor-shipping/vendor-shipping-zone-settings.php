	...
	<form action="" method="post">
		...
		<?php if( $show_post_code_list ) { ?>
			<div class="form-group hide_if_zone_not_limited">
			   	...
			   	<div class="col-md-6 col-sm-9">
			    	<input id="select_zone_postcodes" class="form-control" type="text" name="<?php echo 'wcmp_shipping_zone['. $zone_id .'][_select_zone_postcodes]'; ?>" value="<?php echo !empty( $postcodes ) ? $postcodes : ''; ?>" placeholder="<?php _e( 'Postcodes need to be comma separated', 'dc-woocommerce-multi-vendor' ); ?>">
			   	</div>
			</div>
		<?php } ?>
	</form>
