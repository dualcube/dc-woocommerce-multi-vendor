...
if( $meta_field_data == 'refund_accept' ){
  $status_refund = __( 'Accepted', 'dc-woocommerce-multi-vendor' );
}else{
  $status_refund = __( 'Rejected', 'dc-woocommerce-multi-vendor' );
}
...
<div style="font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; margin-bottom: 40px;">
        ...
        <!-- refund accepted time -->
        <?php }else{  ?>
          <span class="text"><?php printf(__( 'Customer refund request is ' . $status_refund . ' for order : <a href="%s" title="%s">#%s</a> ', 'dc-woocommerce-multi-vendor' ), $order_uri, sanitize_title($order_data->get_status()), $order_data->get_order_number()  ); 
            ?>
          </span>
        <?php } ?>
        <!-- Admin getting this email -->
        <?php } elseif( in_array( 'administrator', $user_roles, true ) ) { 
          ...
          <!-- accepted time getting this email -->
          <?php }else{  ?>
